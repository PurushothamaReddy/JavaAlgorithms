...
public class MergeSort {
    // divide and conquer
    // divide the array until low>=high each element in one division
    // merge by comparing each element
    ...
    public static void sortP(int[] arr, int low, int high) {
        if(low<high) {
            int mid= (low+high)/2;
            sortP(arr,low,mid);
            sortP(arr,mid+1,high);
            mergeP(arr,low,mid,high);
        }
    }

    public static void mergeP(int[] arr, int low, int mid, int high) {
        int n1= mid-low+1;
        int n2= high-mid;
        int[] la = new int[n1];
        int[] ra = new int[n2];
